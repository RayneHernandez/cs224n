...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if padding_length > 0:
            padded_sentence = sentence + ([zero_vector] * padding_length)
            padded_labels = labels + ([zero_label] * padding_length)
            mask = [True] * sentence_length + [False] * padding_length
        else:
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embeddings_variable = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(embeddings_variable, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        shape = tf.shape(x)[0]
        state = tf.zeros([shape, Config.hidden_size])
        ### END YOUR CODE
        ...
        with tf.variable_scope("Hidden_Layer"):
            for time_step in range(self.max_length):
                ...
                _, state = cell(x[:, time_step, :], state, scope="Hidden_Layer")
                state_with_dropout = tf.nn.dropout(state, dropout_rate)
                output = tf.matmul(state_with_dropout, U) + b2
                preds.append(output)
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_mask, labels=labels_mask))
        ### END YOUR CODE
        return loss

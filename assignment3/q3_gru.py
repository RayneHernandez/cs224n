...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        # is True.
        # - Remember to set self.grad_norm
        gradient_variable = optimizer.compute_gradients(loss)
        gradients = [pair[0] for pair in gradient_variable]
        variables = [pair[1] for pair in gradient_variable]
        if self.config.clip_gradients:
            gradients,_ = tf.clip_by_global_norm(gradients, clip_norm=self.config.max_grad_norm)
            
        gradient_variable = [(gradients[i], variables[i]) for i in range(len(gradients))]
        self.grad_norm = tf.global_norm(gradients)
        train_op =  optimizer.apply_gradients(gradient_variable)
        ### END YOUR CODE
